...
Calendar.prototype.init = function () {
    ...
    dayEventcontent.addEventListener("click", function (e) {
        if (e.target && e.target.nodeName == "LI" && e.target.id) {
            let _selectDate = e.target.id.slice(4, 12);
            ...
            if (e.target.className == "add-task-del") {
                ...
                // console.log("_this.dayHaveTask" + _this.dayHaveTask)
            }
        }
    })
    //添加一条todo
...
Calendar.prototype.timeToggle = function () {
    ...
    if (cuHours == 00 && cuMinutes == 00 && cuSeconds == 01) {
        this.dayToggle();
    }
}

//获取月天数
Calendar.prototype.getMonthDays = (year, month) => {
...
//获取当月第一天星期
Calendar.prototype.getfirstDayWeek = (year, month) => {
    const m = new Date(year, month, 01);
    return m.getDay();
}
